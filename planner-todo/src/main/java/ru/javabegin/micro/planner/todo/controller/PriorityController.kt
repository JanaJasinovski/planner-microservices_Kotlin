...
(
    ...
    fun add(@RequestBody priority: Priority): ResponseEntity<Any> {
        ...
        return if (userRestBuilder.userExists(priority.userId!!)) { // вызываем микросервис из другого модуля
            ResponseEntity.ok(priorityService.add(priority)) // возвращаем добавленный объект с заполненным ID
        } else {
            ResponseEntity<Any>("user id=" + priority.userId + " not found", HttpStatus.NOT_ACCEPTABLE) // если пользователя НЕ существует
        }
    ...
    fun search(@RequestBody prioritySearchValues: PrioritySearchValues): ResponseEntity<Any> {
        ...
        } else ResponseEntity.ok(priorityService.find(prioritySearchValues.title, prioritySearchValues.userId))
